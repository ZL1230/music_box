        ...
        <tr v-for="(item, index) in lists" :key="index" class="el-table__row">
          ...
          <td>
            <div class="song-wrap">
              <div class="name-wrap">
                <span>{{ item.name }}</span>
                <span class="iconfont el-icon-video-play" style="color:deeppink;margin-left:10px" v-if="item.mvid!== 0" @click="toMV(item.mvid)"></span>
              </div>
              <span></span>
            </div>
          </td>
    ...
    methods: {
      ...
      getList() {
        ...
        axios({
          ...
          // console.log(res)
          this.lists = res.data.data.splice(0,8)
          // 处理时长 毫秒 转为 分秒
